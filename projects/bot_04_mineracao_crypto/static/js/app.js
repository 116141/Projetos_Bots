...
async function fetchStatus() {
    try {
        ...
        document.getElementById('metricNetDaily').innerText = `$${data.active_net_daily.toFixed(2)} / dia`;
        document.getElementById('metricMonthlyEst').innerText = `$${data.active_monthly_est.toFixed(2)} / mês`;
        document.getElementById('metricPower').innerText = `${data.power_consumption_watts}W (${data.rig_hashrate_mhs} MH/s)`;
        document.getElementById('metricElecCost').innerText = `$${data.electricity_cost_kwh.toFixed(2)} / kWh`;
...
function renderRankings(rankings) {
    ...
    tbody.innerHTML = rankings.map(r => `
        <tr>
            <td><strong>${r.name}</strong> (${r.algo})</td>
            <td style="color: #00f2fe;">$${r.gross_daily.toFixed(2)}</td>
            <td style="color: var(--accent-orange);">$${r.elec_cost_daily.toFixed(2)}</td>
            <td style="color: var(--accent-green);"><strong>+$${r.net_daily.toFixed(2)}</strong></td>
